...
const createUserAction = (userCred) => async (dispatch) => {
  ...
  await axios.post(`http://localhost:3000/api/v1/users?username=${username}&password=${password}&password_confirmation=${passwordConfirmation}`);
  dispatch({
    type: CREATE_USER,
  });
  const response = await axios.post(`http://localhost:3000/api/v1/login?username=${username}&password=${password}`);
  dispatch({
    type: LOGIN,
    username,
